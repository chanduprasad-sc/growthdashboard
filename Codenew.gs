...
.addItem("Open Visual Dashboard", "openVisualDashboard")
...
function openVisualDashboard() {
var url = ScriptApp.getService().getUrl() + '?page=viz';
var html = HtmlService.createHtmlOutput(
'<script>window.open("' + url + '"); google.script.host.close();</script>'
).setWidth(10).setHeight(10);
try { SpreadsheetApp.getUi().showModalDialog(html, "Opening Visual Dashboard…"); } catch(e) {}
}
...
// AI SUMMARY
// ================================================================
function getAISummary(records, dateLabel, filterLabel) {
if (!records || records.length === 0) return { error: "No comments found for the selected date range." };
function fallbackAiSummary(note) {
return buildLocalAiSummary(records, dateLabel, filterLabel, note);
}
function normalizeAiSummaryRecords(rows) {
return (rows || []).map(function(r) {
if (r && typeof r === "object" && !Array.isArray(r)) {
return {
comment: String(r.comment || r.Comment || r.comments || r.Comments || r.Body || r["Last Message"] || r["Agent Messages"] || r["Customer Messages"] || r.Title || "").trim(),
rm: String(r.rm || r["RM Name"] || r["Owner[0]"] || r["Created by"] || "NA").trim(),
branch: String(r.branch || r["Branch/Location"] || r["Location"] || "NA").trim(),
broker: String(r.broker || r["RM Broker Name"] || r["Broker Name[0]"] || "NA").trim(),
issue: String(r.issue || r["Issue Type (B2B)"] || r["Issue"] || "NA").trim(),
subIssue: String(r.subIssue || r["Sub Issue Type (B2B)"] || r["Sub-Issue"] || "NA").trim(),
channel: String(r.channel || r["Channel (B2B)"] || r["Source channel"] || "NA").trim(),
created: String(r.created || r["Created date"] || r["Created"] || "").trim()
};
}
var raw = String(r == null ? "" : r).trim();
if (!raw) return { comment: "", rm: "NA", branch: "NA", broker: "NA", issue: "NA", subIssue: "NA", channel: "NA", created: "" };
var getTag = function(name) {
var m = raw.match(new RegExp("<" + name + ">([\\s\\S]*?)<\\/" + name + ">", "i"));
return m ? String(m[1]).replace(/[\r\n\t]+/g, " ").replace(/\s+/g, " ").trim() : "";
};
var comment = getTag("comment") || raw.replace(/<[^>]+>/g, " ").replace(/[\r\n\t]+/g, " ").replace(/\s+/g, " ").trim();
return {
comment: comment,
rm: getTag("rm_name") || "NA",
branch: getTag("location") || "NA",
broker: getTag("broker") || "NA",
issue: getTag("issue") || "NA",
subIssue: getTag("sub_issue") || "NA",
channel: getTag("channel") || "NA",
created: ""
};
});
}
records = normalizeAiSummaryRecords(records);
  if (!NVIDIA_API_KEY) return fallbackAiSummary("Summary generated from the filtered records.");

  var safeRecords = records.slice(0, 120).map(function(r, i) {
    var comment = String(r && r.comment ? r.comment : "").replace(/[\r\n\t]+/g, " ").replace(/[^\x20-\x7E]/g, "").replace(/"/g, "'").trim().substring(0, 180);
    var rm = String(r && r.rm ? r.rm : "NA").replace(/[^\x20-\x7E]/g, "").trim();
    var branch = String(r && r.branch ? r.branch : "NA").replace(/[^\x20-\x7E]/g, "").trim();
    var broker = String(r && r.broker ? r.broker : "NA").replace(/[^\x20-\x7E]/g, "").trim();
    var issue = String(r && r.issue ? r.issue : "NA").replace(/[^\x20-\x7E]/g, "").trim();
    var subIssue = String(r && r.subIssue ? r.subIssue : "NA").replace(/[^\x20-\x7E]/g, "").trim();
    var channel = String(r && r.channel ? r.channel : "NA").replace(/[^\x20-\x7E]/g, "").trim();
    var created = String(r && r.created ? r.created : "").replace(/[^\x20-\x7E]/g, "").trim();
    return (i + 1) + ". [" + created + "] RM=" + rm + " | Branch=" + branch + " | Broker=" + broker + " | Issue=" + issue + " | SubIssue=" + subIssue + " | Channel=" + channel + " | Comment=" + comment;
  }).join("\n");

  var prompt =
    "You are analysing B2B support ticket and chat comments from smallcase (a fintech platform).\n\n" +
    "Selection date range: " + dateLabel + ".\n" +
    "Applied filters: " + String(filterLabel || "None") + ".\n\n" +
    "RECORDS:\n" + safeRecords + "\n\n" +
    "TASK:\n" +
    "1) Group the records into 4-6 themes.\n" +
    "2) Summarise what the comments are saying.\n" +
    "3) Explicitly mention the recurring RM names, branches, brokers, issue types, sub-issue types and channels that appear to drive the themes.\n" +
    "4) Call out any repeated patterns or operational hotspots.\n\n" +
    "RULES:\n" +
    "- Respond ONLY with raw JSON. No markdown. No code fences.\n" +
    "- Use only ASCII characters.\n" +
    "- The 'narrative' field MUST be formatted directly as a rich HTML string containing 4-5 substantial paragraphs/points detailing key findings, observations, and actionable operational recommendations.\n" +
    "- Use proper HTML tags for structure inside the 'narrative' string: `<h4>` for subheadings, `<ul>` and `<li>` for list bullet items, and `<strong>` to emphasize key RMs, branches, brokers, and issue types. Do NOT escape HTML tags inside the JSON string.\n" +
    '- Structure: {"clusters":[{"theme":"Payment Links","count":12,"examples":["ex1"],"drivers":["RM A | Branch B | Broker C | Issue X / Sub Y"]}],"narrative":"<h4>🔍 Key Observations & Findings</h4><ul><li><strong>Payment Links:</strong> Recurring failures at <strong>Mumbai Branch</strong> driven by <strong>RM John</strong>...</li></ul><h4>💡 Actionable Recommendations</h4><ul><li>Initiate training for...</li></ul>","hotspots":[{"rm":"RM A","branch":"Branch B","broker":"Broker C","issue":"Issue X","subIssue":"Sub Y","channel":"WA","count":4}]}';

  try {
    var resp = UrlFetchApp.fetch("https://integrate.api.nvidia.com/v1/chat/completions", {
    ...
    if (result.error || !result.choices || !result.choices[0] || !result.choices[0].message) return fallbackAiSummary("Summary generated from the filtered records.");
    var text = result.choices[0].message.content.trim().replace(/^```(?:json)?\s*/i, "").replace(/\s*```\s*$/, "").trim();
    var parsed = parseAiSummaryResponse(text);
if (!parsed) parsed = repairAiSummaryResponse(text, dateLabel, filterLabel);
if (!parsed) parsed = fallbackAiSummary("Summary generated from the filtered records.");
...
return fallbackAiSummary("Summary generated from the filtered records.");
}
}
...
function repairAiSummaryResponse(rawText, dateLabel, filterLabel) {
  if (!NVIDIA_API_KEY) return null;
  var repairInput = String(rawText || '').substring(0, 8000);
  var prompt =
    "You are repairing a malformed AI summary for a dashboard.\n\n" +
    "Context date range: " + dateLabel + ".\n" +
    "Applied filters: " + String(filterLabel || "None") + ".\n\n" +
    "Return ONLY valid raw JSON using exactly this shape:\n" +
    '{"clusters":[{"theme":"string","count":1,"examples":["string"],"drivers":["string"]}],"narrative":"string","hotspots":[{"rm":"string","branch":"string","broker":"string","issue":"string","subIssue":"string","channel":"string","count":1}]}\n\n' +
    "Rules:\n" +
    "- Use ASCII only.\n" +
    "- Do not add markdown or code fences.\n" +
    "- Do not add any extra keys.\n" +
    "- Preserve the meaning of the source text, specifically keeping all HTML formatting tags like <h4>, <ul>, <li>, and <strong> inside the narrative string intact.\n\n" +
    "SOURCE TEXT:\n" + repairInput;
  ...
  try {
    ...
    var result = JSON.parse(resp.getContentText());
    if (result.error || !result.choices || !result.choices[0] || !result.choices[0].message) return null;
    var text = String(result.choices[0].message.content || '').trim().replace(/^```(?:json)?\s*/i, "").replace(/\s*```\s*$/, "").trim();
    var parsed = parseAiSummaryResponse(text);
...
function buildLocalAiSummary(records, dateLabel, filterLabel, note) {
var themeMap = {};
var hotspotMap = {};
var brokerMap = {};
var branchMap = {};
...
var rm = String(r && r.rm ? r.rm : "NA");
var branch = String(r && r.branch ? r.branch : "NA");
var broker = String(r && r.broker ? r.broker : "NA");
var issue = String(r && r.issue ? r.issue : "NA");
var subIssue = String(r && r.subIssue ? r.subIssue : "NA");
var channel = String(r && r.channel ? r.channel : "NA");
var comment = String(r && r.comment ? r.comment : "").trim();
var key = issue + (subIssue !== "NA" ? " / " + subIssue : "");
var hKey = [rm, branch, broker, issue, subIssue, channel].join("||");

if (issue !== "NA") issueMap[issue] = (issueMap[issue] || 0) + 1;
if (subIssue !== "NA") subIssueMap[subIssue] = (subIssueMap[subIssue] || 0) + 1;
if (rm !== "NA") rmMap[rm] = (rmMap[rm] || 0) + 1;
if (branch !== "NA") branchMap[branch] = (branchMap[branch] || 0) + 1;
if (broker !== "NA") brokerMap[broker] = (brokerMap[broker] || 0) + 1;
if (channel !== "NA") channelMap[channel] = (channelMap[channel] || 0) + 1;

if (!hotspotMap[hKey]) hotspotMap[hKey] = { rm: rm, branch: branch, broker: broker, issue: issue, subIssue: subIssue, channel: channel, count: 0, examples: [] };
hotspotMap[hKey].count++;
if (comment && hotspotMap[hKey].examples.length < 2) hotspotMap[hKey].examples.push(comment.substring(0, 160));

if (!themeMap[key]) themeMap[key] = { theme: key === "NA" ? "General" : key, count: 0, examples: [], drivers: [] };
themeMap[key].count++;
if (comment && themeMap[key].examples.length < 2) themeMap[key].examples.push(comment.substring(0, 160));
if (themeMap[key].drivers.length < 2) {
themeMap[key].drivers.push([rm, branch, broker, issue + (subIssue !== "NA" ? " / " + subIssue : "")].join(" | "));
}

function normalizeAiSummaryRecords(records) {
...
return Object.keys(map).sort(function(a, b) { return map[b] - map[a]; }).slice(0, limit).map(function(k) {
return { key: k, count: map[k] };
});
}

var clusters = topEntries(themeMap, 6).map(function(item) {
var theme = themeMap[item.key];
return { theme: theme.theme, count: theme.count, examples: theme.examples, drivers: theme.drivers };
});

var hotspots = Object.keys(hotspotMap).sort(function(a, b) { return hotspotMap[b].count - hotspotMap[a].count; }).slice(0, 5).map(function(k) {
var hs = hotspotMap[k];
return { rm: hs.rm, branch: hs.branch, broker: hs.broker, issue: hs.issue, subIssue: hs.subIssue, channel: hs.channel, count: hs.count, examples: hs.examples };
});

var topIssue = topEntries(issueMap, 1)[0];
var topBranch = topEntries(branchMap, 1)[0];
var topBroker = topEntries(brokerMap, 1)[0];
var topRm = topEntries(rmMap, 1)[0];
var topChannel = topEntries(channelMap, 1)[0];

var narrativeParts = [];
narrativeParts.push("<h4>🔍 Key Observations & Findings</h4>");
narrativeParts.push("<ul>");
if (topIssue) narrativeParts.push("<li><strong>Leading Driver:</strong> <code>" + topIssue.key + "</code> was reported <strong>" + topIssue.count + "</strong> times in the filtered slice.</li>");
if (topBranch) narrativeParts.push("<li><strong>Busiest Location:</strong> Branch <strong>" + topBranch.key + "</strong> generated the highest interaction volume.</li>");
if (topBroker) narrativeParts.push("<li><strong>Top Broker Partner:</strong> <strong>" + topBroker.key + "</strong> was the primary source of broker support queries.</li>");
if (topRm) narrativeParts.push("<li><strong>Most Active RM:</strong> <strong>" + topRm.key + "</strong> managed the largest volume of support tickets.</li>");
if (topChannel) narrativeParts.push("<li><strong>Dominant Support Channel:</strong> Support tickets were predominantly routed through <strong>" + topChannel.key + "</strong>.</li>");
narrativeParts.push("</ul>");

narrativeParts.push("<h4>💡 Actionable Recommendations</h4>");
narrativeParts.push("<ul>");
if (topIssue) narrativeParts.push("<li>Analyze root causes for the dominant issue category <strong>" + topIssue.key + "</strong> to automate common responses.</li>");
if (topRm) narrativeParts.push("<li>Review support load allocation for RM <strong>" + topRm.key + "</strong> to balance agent bandwidth.</li>");
narrativeParts.push("<li>Conduct regional check-ins with <strong>" + (topBranch ? topBranch.key : "active") + "</strong> branch stakeholders to resolve systemic bottlenecks.</li>");
narrativeParts.push("</ul>");

if (note) narrativeParts.push("<p style='margin-top:12px; font-size:11px; font-style:italic;'>Note: " + note + "</p>");
